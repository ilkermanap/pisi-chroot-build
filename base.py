...
class Chroot:
    ...
    def cleanDocs(self, clearDbus = True):
        if self.root !="":
            ...
            self.runOutside("rm -rf %s/usr/lib/locale/locale-archive" % self.root)
            self.runOutside("rm -rf %s/usr/local" % self.root)
            self.runOutside("rm -rf %s/usr/lib/python2.7/test" % self.root)
